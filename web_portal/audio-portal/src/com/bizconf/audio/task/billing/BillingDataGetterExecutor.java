...
public class BillingDataGetterExecutor {
	...
	private static final long REST_INTERVAL = 2*24*3600*1000L;// 两天来一次？？
			...
			try {
				...
				if(dir.exists()){
					...
					for (int i = 0; i < files.length; i++) {
						if(files[i].getName().equals("")){ //TODO 这里需要把CDR账单命名加上去
							cdrFile = files[i];
						}
						if(files[i].getName().equals("")){ //TODO 这里需要把站点月租用费账单命名加上去
							rentFile = files[i];
						}
					}
				}
