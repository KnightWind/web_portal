	          		...
	          		<tr>
	          			<td class="" ><!-- ${LANG['bizconf.jsp.conf_list_index.res53']} -->
	          				<c:if test="${user.id eq newConfBase.compereUser && newConfBase.confStatus==CONF_STATUS_SUCCESS}">
	          					<div class="k_f" onclick="inventContact(${newConfBase.id})"><img src="/static/images/yaoqing.png" width="21" height="16" align="absmiddle" style=" padding-right:5px;" />${LANG['bizconf.jsp.conf_list_index.res13']}</div>
          					</c:if>
          				</td>
	            			...
	            			<div class="k06" onclick="javascript:joinMeeting(1,'${newConfBase.id}');">
	            				<c:if test="${newConfBase.confStatus==CONF_STATUS_FINISHED}">
	            					<c:if test="${user.id eq newConfBase.createUser}">
							              <a href="javascript:;" onclick="reCreateReservationConf(${newConfBase.id})">${LANG['bizconf.jsp.attended_conf_list.res11']}</a>
						            </c:if>
	            				</c:if>
	            				<c:if test="${newConfBase.confStatus!=CONF_STATUS_FINISHED}">
